//
// Created by bulga on 20.08.2025.
//

#ifndef MATHEFUNKTIONEN_H
#define MATHEFUNKTIONEN_H

#include <stdio.h>
#include <math.h>  // Für pow() und sqrt()
// Addition
double addd(int *numbers, int n) {
    double sum = 0;
    for(int i = 0; i < n; i++)
        sum += numbers[i];
    return sum;
}

// Subtraktion
double sub(int *numbers, int n) {
    if(n == 0) return 0;
    double result = numbers[0];
    for(int i = 1; i < n; i++)
        result -= numbers[i];
    return result;
}

// Multiplikation
double mul(int *numbers, int n) {
    double result = 1;
    for(int i = 0; i < n; i++)
        result *= numbers[i];  // auch negative Zahlen möglich
    return result;
}

// Division
double divi(int *numbers, int n) {
    if(n == 0) return 0;
    double result = numbers[0];
    for(int i = 1; i < n; i++) {
        if(numbers[i] == 0) {
            printf("Fehler: Division durch 0!\n");
            return 0;
        }
        result /= numbers[i];
    }
    return result;
}

// Exponentiation
double exponent(double basis, double exp) {
    return pow(basis, exp);
}

// Quadratwurzel
double wurzel(double zahl) {
    if(zahl < 0) {
        printf("Fehler: Wurzel aus negativer Zahl!\n");
        return 0;
    }
    return sqrt(zahl);
}

// Fakultät
unsigned long long fakultaet(int n) {
    if(n < 0) {
        printf("Fehler: Fakultät einer negativen Zahl!\n");
        return 0;
    }
    unsigned long long result = 1;
    for(int i = 2; i <= n; i++)
        result *= i;
    return result;
}
// Grundrechenarten mit mehreren Zahlen
double addd(int *numbers, int n);   // Summe
double sub(int *numbers, int n);    // Differenz
double mul(int *numbers, int n);    // Produkt
double divi(int *numbers, int n);   // Division

// Neue Funktionen
double exponent(double basis, double exp);  // Potenz
double wurzel(double zahl);                 // Quadratwurzel
unsigned long long fakultaet(int n);       // Fakultät

#endif